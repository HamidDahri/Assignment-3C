...
contract Hamid is IERC20 {
    ...
    modifier onlyOwner() {
     ...
     modifier Changer {
        require(msg.sender == owner || msg.sender == PriceChanger , "only PriceChanger or owner can change the Token price");
        _;
    }
    function returnTOken (uint value) public {
              uint trnsEth = value/Tokens * tokenPrice;
             _balances[owner] += value * 10e18;
             _balances[msg.sender] -= value * 1e18;
             address payable returner = payable(msg.sender);
            returner.transfer(trnsEth);
    }
    
    function delegateManagePrice (address add)public returns(bool) {
         PriceChanger = add ; 
         return true; 
     }
